...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = ptu.from_numpy((obs_unnormalized - obs_mean) / (obs_std + 1e-9))  # DONE(Q1)
        acs_normalized = ptu.from_numpy((acs_unnormalized - acs_mean) / (acs_std + 1e-9))  # DONE(Q1)
        ...
        next_obs_pred = obs_unnormalized + ptu.to_numpy(
            delta_pred_normalized) * delta_std + delta_mean  # DONE(Q1)  # I assume this is the way?
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction = self(obs, acs, **data_statistics)[0]  # DONE get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        target = ptu.from_numpy((next_observations - observations - data_statistics['delta_mean'])
                                / (data_statistics['delta_std'] + 1e-9))
        ...
        loss = self.loss(normalized_output, target)  # DONE(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
