...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        assert not torch.is_tensor(obs_unnormalized)
        assert not torch.is_tensor(acs_unnormalized)
        obs_normalized = ptu.from_numpy((obs_unnormalized - obs_mean) / (obs_std + 1e-9))  # TODO(Q1)
        acs_normalized = ptu.from_numpy((acs_unnormalized - acs_mean) / (acs_std + 1e-9))  # TODO(Q1)
        ...
        next_obs_pred = obs_unnormalized + ptu.to_numpy(
            delta_pred_normalized) * delta_std + delta_mean  # TODO(Q1)  # I assume this is the way?
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction = self(obs, acs, **data_statistics)[0]  # TODO get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        target = ptu.from_numpy((next_observations - observations - data_statistics['delta_mean']) / (data_statistics[
            'delta_std'] + 1e-9))  # TODO(Q1) compute the normalized target for the model.
        # # Hint: you should use `data_statistics['delta_mean']` and
        # # `data_statistics['delta_std']`, which keep track of the mean
        # # and standard deviation of the model.

        normalized_output = self(observations, actions, **data_statistics)[1]
        assert normalized_output.shape == target.shape
        loss = self.loss(normalized_output, target)  # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
